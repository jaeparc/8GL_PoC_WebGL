...
public class CampusInfos : MonoBehaviour
{
    ...
    public void DisplayCampusInfo(string id)
    {
        foreach (var item in CampusList)
        {
            if (item.ItemId == id)
            {
                ...
                StartCoroutine(LoadImage("https://cdn.filestackcontent.com/AEWX4Afq3SMm2gtuHsepBz/"+item.ItemImageUrl));
                return;
            }
        }
    }
    ...
    private IEnumerator LoadImage(string url)
    {
        ...
        {
            Debug.LogError("# WEBFILE # Chargement image en erreur: " + url + " avec message erreur " + www.error);
        }
        else
        {
